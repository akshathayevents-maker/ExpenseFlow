...
<x-ds.hero eyebrow="Leave Management" title="Leave Requests"
    ...
    <x-slot:actions>
        <a href="{{ route('admin.leave-types.index') }}" class="ef-ds-btn">
            <i class="bi bi-sliders"></i> <span>Leave Types</span>
        </a>
    </x-slot:actions>
</x-ds.hero>
...
<div class="lvr-filter-row">
    <a href="{{ route('admin.leave.requests.index') }}" class="lvr-filter-chip {{ $status === '' ? '--active' : '' }}">All</a>
    <a href="{{ route('admin.leave.requests.index', ['status' => 'pending']) }}" class="lvr-filter-chip {{ $status === 'pending' ? '--active' : '' }}">Pending</a>
    <a href="{{ route('admin.leave.requests.index', ['status' => 'approved']) }}" class="lvr-filter-chip {{ $status === 'approved' ? '--active' : '' }}">Approved</a>
    <a href="{{ route('admin.leave.requests.index', ['status' => 'rejected']) }}" class="lvr-filter-chip {{ $status === 'rejected' ? '--active' : '' }}">Rejected</a>
    <a href="{{ route('admin.leave.requests.index', ['status' => 'cancelled']) }}" class="lvr-filter-chip {{ $status === 'cancelled' ? '--active' : '' }}">Cancelled</a>
</div>
...
<x-ds.card title="Leave Requests" :no-pad="true">
    <div class="lvr-list" style="padding:14px">
        @forelse($requests as $leaveRequest)
            ...
            <a href="{{ route('admin.leave.requests.show', $leaveRequest) }}" class="lvr-row">
