...
<div class="d-md-none"
     x-data="{
         ...
         go()  { this.ld=true; const p=new URLSearchParams(); if(this.f) p.set('from',this.f); if(this.t) p.set('to',this.t); window.location.href='{{ route('admin.reports.employee') }}'+(p.size?'?'+p:'') },
         rst() { this.ld=true; window.location.href='{{ route('admin.reports.employee') }}' }
     }">
    ...
    <div class="ef-rpf-wrap">
        ...
        <div class="ef-rpf-grid-2">
            <div>
                ...
                <input type="date" class="ef-rpf-date" x-model="f" @change="f0!==f ? null : null">
            </div>
            <div>
    ...
    <div style="overflow-x:auto">
        <table class="ef-an-trend-table">
            ...
            <tbody>
                ...
                @empty
                <tr><td colspan="6" style="text-align:center;padding:32px;color:var(--ef-faint)">
                    <i class="bi bi-person-lines-fill" style="font-size:1.5rem;display:block;margin-bottom:8px;opacity:.3"></i>
                    No data{{ $hasFilters ? ' for selected period' : '' }}.
                    @if($hasFilters)
                        <a href="{{ route('admin.reports.employee') }}" style="display:block;margin-top:8px;font-size:.8rem;color:var(--ef-emerald)">Clear filters</a>
                    @endif
                </td></tr>
                @endforelse
            </tbody>
