...
<div class="d-md-none"
     x-data="{
         ...
         go()  { this.ld=true; const p=new URLSearchParams(); if(this.f) p.set('from',this.f); if(this.t) p.set('to',this.t); window.location.href='{{ route('admin.reports.category') }}'+(p.size?'?'+p:'') },
         rst() { this.ld=true; window.location.href='{{ route('admin.reports.category') }}' }
     }">
        ...
        <table class="ef-an-trend-table">
            ...
            <tbody>
                ...
                <tr><td colspan="5" style="text-align:center;padding:32px;color:var(--ef-faint)">
                    <i class="bi bi-tag" style="font-size:1.5rem;display:block;margin-bottom:8px;opacity:.3"></i>
                    No data{{ $hasFilters ? ' for selected period' : '' }}.
                    @if($hasFilters)
                        <a href="{{ route('admin.reports.category') }}" style="display:block;margin-top:8px;font-size:.8rem;color:var(--ef-emerald)">Clear filters</a>
                    @endif
                </td></tr>
                @endforelse
            </tbody>
