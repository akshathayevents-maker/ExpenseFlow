...
<style>
/*
 * ADMIN EXPENSE REQUESTS — ef-mgr-* namespace (mirrors manager design system)
 */
...
.ef-mgr-hero-stat-val.teal   { color: #5eead4; }
...
.ef-mgr-req-card[data-status="pending_payment"]       { border-left-color: var(--mgr-blue); }
.ef-mgr-req-card[data-status="reimbursement_pending"] { border-left-color: var(--mgr-indigo); }
...
.ef-mgr-status-chip.pending_payment       { background: #eff6ff; color: var(--mgr-blue); }
.ef-mgr-status-chip.reimbursement_pending { background: #eef2ff; color: var(--mgr-indigo); }
...
@media (max-width: 991.98px) {
    ...
    .ef-mgr-adv-row { grid-template-columns: 1fr 1fr 1fr; }
}
...
@php
    ...
    $activeTo       = $filters['to']          ?? '';
    $activeEmployee = $filters['employee_id'] ?? '';
    $activeCategory = $filters['category_id'] ?? '';
    $hasAdvFilter   = $activeCategory || $activeEmployee || $activeFrom || $activeTo || $activePriority;
    ...
    $statusTimelines = [
        ...
        'pending_payment'       => [['done','Submitted'],['done','Review'],['done','Approved'],['active','Paid']],
        'reimbursement_pending' => [['done','Submitted'],['done','Review'],['done','Approved'],['active','Paid']],
    ];
...
<section class="ef-mgr-hero">
    <div class="ef-mgr-hero-main">
        <div class="ef-mgr-eyebrow">Admin Operations Center</div>
        <h1 class="ef-mgr-hero-title">Expense Requests</h1>
        <div class="ef-mgr-hero-sub">Full-scope expense management, approvals and settlement tracking</div>
    </div>
    <div class="ef-mgr-hero-stats">
        <div class="ef-mgr-hero-stat">
            <div class="ef-mgr-hero-stat-val {{ $stats['pending_count'] > 0 ? 'amber' : '' }}">
                {{ $stats['pending_count'] }}
            </div>
        ...
        <div class="ef-mgr-hero-stat">
            <div class="ef-mgr-hero-stat-val {{ $stats['rejected_count'] > 0 ? 'danger' : '' }}">
                {{ $stats['rejected_count'] }}
            </div>
            <div class="ef-mgr-hero-stat-lbl">Rejected</div>
        </div>
        <div class="ef-mgr-hero-divider"></div>
        <div class="ef-mgr-hero-stat">
            <div class="ef-mgr-hero-stat-val teal" style="font-size:1.2rem">
                {{ $fmt($stats['total_amount']) }}
            </div>
            <div class="ef-mgr-hero-stat-lbl">Total Value</div>
        </div>
    </div>
</section>
...
<div class="ef-mgr-kpi-wrap">
    <div class="ef-mgr-kpi-strip">
        <a href="{{ route('admin.expense-requests.index', ['status' => 'pending']) }}" class="ef-mgr-kpi">
            <div class="ef-mgr-kpi-icon"><i class="bi bi-hourglass-split"></i></div>
            <div class="ef-mgr-kpi-label">
                @if($stats['pending_count'] > 0)<span class="ef-mgr-kpi-pulse"></span>@endif
                Pending
            </div>
            <div class="ef-mgr-kpi-value {{ $stats['pending_count'] > 0 ? 'amber' : 'emerald' }}">
                {{ $stats['pending_count'] }}
            </div>
            <div class="ef-mgr-kpi-note">awaiting review</div>
        </a>
        <a href="{{ route('admin.expense-requests.index', ['status' => 'approved']) }}" class="ef-mgr-kpi">
            <div class="ef-mgr-kpi-icon"><i class="bi bi-check-circle-fill"></i></div>
            <div class="ef-mgr-kpi-label">Approved</div>
            <div class="ef-mgr-kpi-value emerald">{{ $stats['approved_count'] }}</div>
            <div class="ef-mgr-kpi-note">awaiting settlement</div>
        </a>
        <a href="{{ route('admin.expense-requests.index', ['status' => 'rejected']) }}" class="ef-mgr-kpi">
            <div class="ef-mgr-kpi-icon"><i class="bi bi-x-circle-fill"></i></div>
            <div class="ef-mgr-kpi-label">Rejected</div>
            <div class="ef-mgr-kpi-value danger">{{ $stats['rejected_count'] }}</div>
            <div class="ef-mgr-kpi-note">all time</div>
        </a>
        <a href="{{ route('admin.expense-requests.index', ['status' => 'paid']) }}" class="ef-mgr-kpi">
            <div class="ef-mgr-kpi-icon"><i class="bi bi-check2-all"></i></div>
            <div class="ef-mgr-kpi-label">Settled</div>
            <div class="ef-mgr-kpi-value teal">{{ $stats['settled_count'] }}</div>
            <div class="ef-mgr-kpi-note">paid / reimbursed</div>
        </a>
        <div class="ef-mgr-kpi">
            <div class="ef-mgr-kpi-icon"><i class="bi bi-currency-rupee"></i></div>
            <div class="ef-mgr-kpi-label">Total Value</div>
            <div class="ef-mgr-kpi-value blue" style="font-size:1.1rem">{{ $fmt($stats['total_amount']) }}</div>
            <div class="ef-mgr-kpi-note">in current view</div>
        </div>
        <div class="ef-mgr-kpi">
            <div class="ef-mgr-kpi-icon"><i class="bi bi-calendar-month"></i></div>
            <div class="ef-mgr-kpi-label">This Month</div>
            <div class="ef-mgr-kpi-value indigo" style="font-size:1.1rem">{{ $fmt($stats['monthly_total']) }}</div>
            <div class="ef-mgr-kpi-note">excl. rejected</div>
        </div>
    </div>
</div>

{{-- ── Filter Toolbar ──────────────────────────────────────────── --}}
<form method="GET" id="filterForm" action="{{ route('admin.expense-requests.index') }}">
    <div class="ef-mgr-toolbar">
        ...
        <div class="ef-mgr-chips">
            ...
            <a href="{{ route('admin.expense-requests.index', ['status' => 'pending']) }}"
               class="ef-mgr-chip ef-mgr-chip-amber {{ $activeStatus === 'pending' ? 'active' : '' }}">
                ...
                @if($stats['pending_count'] > 0)
                    <span class="ef-mgr-chip-count">{{ $stats['pending_count'] }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.expense-requests.index', ['status' => 'rejected']) }}"
               class="ef-mgr-chip ef-mgr-chip-danger {{ $activeStatus === 'rejected' ? 'active' : '' }}">
                ...
                @if($stats['rejected_count'] > 0)
                    <span class="ef-mgr-chip-count">{{ $stats['rejected_count'] }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.expense-requests.index', ['status' => 'reimbursement_pending']) }}"
               class="ef-mgr-chip {{ $activeStatus === 'reimbursement_pending' ? 'active' : '' }}">
                <i class="bi bi-arrow-return-left"></i> Reimb. Pending
            </a>
            <a href="{{ route('admin.expense-requests.index', ['priority' => 'urgent']) }}"
               class="ef-mgr-chip ef-mgr-chip-danger {{ $activePriority === 'urgent' ? 'active' : '' }}">
                <i class="bi bi-fire"></i> Urgent
            </a>
            <a href="{{ route('admin.expense-requests.index', ['priority' => 'high']) }}"
               class="ef-mgr-chip ef-mgr-chip-amber {{ $activePriority === 'high' ? 'active' : '' }}">
                <i class="bi bi-arrow-up-circle"></i> High
            </a>
            <a href="{{ route('admin.expense-requests.index', ['from' => $todayStr, 'to' => $todayStr]) }}"
               class="ef-mgr-chip {{ $activeFrom === $todayStr && $activeTo === $todayStr ? 'active' : '' }}">
                <i class="bi bi-calendar-day"></i> Today
            </a>
            <a href="{{ route('admin.expense-requests.index', ['from' => $weekStartStr]) }}"
        ...
        <div class="ef-mgr-adv-drawer {{ $hasAdvFilter ? 'open' : '' }}" id="advDrawer">
            <div class="ef-mgr-adv-row">
                ...
                <div>
                    <div class="ef-mgr-adv-label">Employee</div>
                    <select name="employee_id" class="ef-mgr-adv-select">
                        <option value="">All employees</option>
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ $activeEmployee == $emp->id ? 'selected' : '' }}>
                                {{ $emp->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="ef-mgr-adv-actions">
                    ...
                    <a href="{{ route('admin.expense-requests.index') }}" class="ef-mgr-adv-reset">Reset</a>
                </div>
            </div>
        </div>
    </div>
...
<div class="ef-mgr-meta">
    <span>
        ...
        @if($activeStatus || $activePriority || $activeFrom || !empty($filters['search']) || $activeEmployee || $activeCategory)
            · <a href="{{ route('admin.expense-requests.index') }}"
            ...
            <div class="ef-mgr-req-card" data-status="{{ $status }}">
                <div class="ef-mgr-req-body">
                    ...
                    <div class="ef-mgr-req-main">
                        ...
                        <div class="ef-mgr-req-chips">
                            @if($req->category)
                                <span class="ef-mgr-cat-chip">
                                    <i class="bi bi-tag" style="font-size:.6rem"></i> {{ $req->category->name }}
                                </span>
                            @endif
                            ...
                            <span class="ef-mgr-status-chip {{ $status }}">{{ $statusLbl }}</span>
                            @if($hasBills)
                                <span class="ef-mgr-bills-chip">
                ...
                <div class="ef-mgr-req-footer">
                    ...
                    <div class="ef-mgr-req-actions">
                        <a href="{{ route('admin.expense-requests.show', $req) }}" class="ef-mgr-btn-view">
                            <i class="bi bi-eye"></i>
                            {{ $isPending ? 'Review' : 'View' }}
                        </a>
                        @if($isPending)
                            <form method="POST"
                                  action="{{ route('admin.expense-requests.approve', $req) }}"
                            ...
                            <button type="button"
                                    ...
                                    onclick="openRejectModal({{ $req->id }}, '{{ addslashes(Str::limit($req->title, 40)) }}', '{{ route('admin.expense-requests.reject', $req) }}')">
...
@else
    ...
    <div class="ef-mgr-empty">
        <div class="ef-mgr-empty-orb">
            <i class="bi bi-{{ $activeStatus || $activePriority || !empty($filters['search']) ? 'search' : 'file-earmark-text' }}"></i>
        </div>
        @if($activeStatus || $activePriority || !empty($filters['search']) || $activeEmployee || $activeCategory)
        ...
        @else
            <h5>No expense requests</h5>
            <p>Employee reimbursement requests and operational expenses will appear here.</p>
        @endif
    </div>
@endif
        ...
        <div class="modal-content">
            ...
            <form id="rejectForm" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <label class="ef-mgr-adv-label" style="margin-bottom:6px">
                        Reason for rejection <span style="color:var(--mgr-danger)">*</span>
                    </label>
